...
namespace Application.Command.Authorization.PassportHolder.Update
{
    internal sealed class UpdatePassportHolderAuthorization : IAuthorization<UpdatePassportHolderCommand>
    {
        ...
        public UpdatePassportHolderAuthorization(IPassportVisaRepository repoVisa)
        {
            this.repoVisa = repoVisa;
        }

        async ValueTask<IMessageResult<bool>> IAuthorization<UpdatePassportHolderCommand>.AuthorizeAsync(UpdatePassportHolderCommand msgMessage, IEnumerable<Guid> enumPassportVisaId, CancellationToken tknCancellation)
        {
