...
namespace Application.Command.Authorization.PassportToken.EnableTwoFactorAuthentication
{
    internal sealed class EnableTwoFactorAuthenticationAuthorization : IAuthorization<EnableTwoFactorAuthenticationCommand>
	{
		...
		public EnableTwoFactorAuthenticationAuthorization(IPassportVisaRepository repoVisa)
		{
			this.repoVisa = repoVisa;
		}

		async ValueTask<IMessageResult<bool>> IAuthorization<EnableTwoFactorAuthenticationCommand>.AuthorizeAsync(EnableTwoFactorAuthenticationCommand msgMessage, IEnumerable<Guid> enumPassportVisaId, CancellationToken tknCancellation)
		{
