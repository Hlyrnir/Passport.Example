...
namespace Application.Command.PhysicalData.PhysicalDimension.Delete
{
	internal sealed class DeletePhysicalDimensionAuthorization : IAuthorization<DeletePhysicalDimensionCommand>
	{
		...
		public DeletePhysicalDimensionAuthorization(
			IPassportVisaRepository repoVisa)
		{
			this.repoVisa = repoVisa;
		}

		async ValueTask<IMessageResult<bool>> IAuthorization<DeletePhysicalDimensionCommand>.AuthorizeAsync(DeletePhysicalDimensionCommand msgMessage, IEnumerable<Guid> enumPassportVisaId, CancellationToken tknCancellation)
		{
